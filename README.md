...
| BaseLineAR       | Временные ряды        | Временные ряды        | `VARIMA(maxlags=5), SARIMAX(order=(1,0,1))`     | `R2: -2.78, MSE: 496007913, MAE: 17613`                       | Простота, скорость обучения   | Низкая точность, чувствительность к шуму     |
