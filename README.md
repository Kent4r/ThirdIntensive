...
| Stationarity       | Временные ряды        | Временные ряды        | `VARIMA(maxlags=5), SARIMAX(order=(1,0,1))`     | `R2, MAE, MSE, RMSE`                            | Подходит для временных рядов   | Чувствительна к сезонности      |
